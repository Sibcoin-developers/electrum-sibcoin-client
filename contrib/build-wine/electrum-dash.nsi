...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-SIB"
...
;Interface Settings
  ...
  !define MUI_ICON "icons\electrum-sib.ico"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-sib.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-sib.exe" "" "$INSTDIR\electrum-sib.exe" 0

  ;Links sib: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\sib" "" "URL:sib Protocol"
  WriteRegStr HKCU "Software\Classes\sib" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\sib" "DefaultIcon" "$\"$INSTDIR\electrum-sib.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\sib\shell\open\command" "" "$\"$INSTDIR\electrum-sib.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrum-sib.ico"
