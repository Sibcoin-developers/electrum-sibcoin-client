...
from electrum_sib.util import user_dir, print_error, print_msg, print_stderr, PrintError

SYSTEM_CONFIG_PATH = "/etc/electrum-sib.conf"
...
def read_system_config(path=SYSTEM_CONFIG_PATH):
    """Parse and return the system config settings in /etc/electrum-sib.conf."""
    result = {}
    if os.path.exists(path):
        try:
            import ConfigParser
        except ImportError:
            print "cannot parse electrum-sib.conf. please install ConfigParser"
            return
...
def read_user_config(path):
    """Parse and store the user config settings in electrum-sib.conf into user_config[]."""
    if not path:
        return {}
